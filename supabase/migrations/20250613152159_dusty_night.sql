/*
  ...
  1. Schema Updates
    - Add `default_budget_amount` to categories table for templates
    - Create `monthly_budgets` table for month-specific budget planning
    
  2. Security
    - Enable RLS on monthly_budgets table
    - Add policy for users to manage their own monthly budgets
    
  3. Functions
    - Function to create monthly budgets from category templates
...
BEGIN
  ...
  LOOP
    ...
    inserted_count := inserted_count + current_row_count;
  END LOOP;
